...
class NativeDetectionDepthModule(reactContext: ReactApplicationContext) : NativeDetectionDepthSpec(reactContext) {
    ...
    override fun analyzeImage(base64Image: String, promise: Promise) {
        try {
            ...
            val yoloOutput = Array(1) { FloatArray(25200) }
            val midasOutput = Array(1) { Array(256) { FloatArray(256) } }
            ...
            Log.e("NativeDetectionDepth", "Error analyzing image", e)
    ...
    private fun preprocessImage(bitmap: Bitmap, width: Int, height: Int): ByteBuffer {
        ...
        val byteBuffer = ByteBuffer.allocateDirect(1 * width * height * 3 * 4) // Correção do tamanho
        ...
        Log.d("NativeDetectionDepth", "Preprocess Image: Buffer Size = ${byteBuffer.capacity()}")
        return byteBuffer
    }
